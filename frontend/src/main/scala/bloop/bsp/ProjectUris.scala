...
object ProjectUris {
  ...
  def parseUri(projectUri: String): Either[String, ParsedProjectUri] = {
    ...
    def invalidFormatMessage: String = s"URI '${projectUri}' has invalid format. Example: ${ProjectUris.Example}"

    def findName(uri: URI): Either[String, String] = {
      val kv = uri.getRawQuery.split("&")
      val maybeName = kv.map(_.split("=")).find {
    ...
    if (projectUri.isEmpty) Left("URI cannot be empty.")
      ...
      Try(new URI(projectUri)).toEither.left.map(_ => invalidFormatMessage)
        .flatMap(uri => findName(uri).map(name => ParsedProjectUri(AbsolutePath.completelyUnsafe(uri.getPath), name)))
    }
  }
